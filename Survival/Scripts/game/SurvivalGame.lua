...
function SurvivalGame.cl_onNewBlueprint( self, params)
	self.network:sendToServer( "sv_saveBlueprint", { name = params[2] } )
	print("sets blueprint name :"..params[2])
end

function SurvivalGame.sv_saveBlueprint( self, params )
	self.sv.blueprints = sm.storage.load( STORAGE_CHANNEL_BLUEPRINTS )
	if self.sv.blueprints then
	...
	self.sv.blueprints.name = params.name
	sm.storage.save( STORAGE_CHANNEL_BLUEPRINTS, self.sv.blueprints )
end
